...
function selectCategory() {
    ...
    let options;
    if(selcat.value === 0)
        options = document.querySelectorAll(".selection");
    else
...
function submitSelect() {
    ...
    postRequest(id, quantity);
}

function submitSearch() {
    let searchtext = document.querySelector("#myInput").value;
    let index = source.itemnames.findIndex(name => name === searchtext);
    if (index === -1) {
        qstatus.classList.add("text-danger");
        qstatus.classList.remove("text-success");
        qstatus.textContent = "Item name does not match!";
        return;
    }
    let id = source.itemids[index];
    let quantity = document.querySelector("#schquantity").value;
    postRequest(id, quantity);
}

function postRequest(id, quantity) {
    fetch("submit.php", {
        method: "POST",
        headers: {
            "Content-Type": "application/x-www-form-urlencoded",
        },
        body: "action=2&id=" + id + "&quantity=" + quantity
...
fetch("submit.php", {
    ...
    .then(text => {
        if (text !== "There are no items!") {
            ...
            autocomplete(document.getElementById("myInput"), source.itemnames);
        }
        else {
            qstatus.classList.add("text-danger");
            qstatus.classList.remove("text-success");
